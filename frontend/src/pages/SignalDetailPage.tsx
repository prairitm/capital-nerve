...
export function SignalDetailPage() {
  ...
  const unmatchedEvidence = useMemo(() => {
    if (!data) return [];
    const used = new Set<number>();
    const touch = (items: EvidenceItem[]) => {
      for (const item of items) used.add(item.card_evidence_id);
    };

    for (const leaf of data.rule_leaves ?? []) {
      touch(evidenceMatchingLabel(data.evidence, leaf.metric_code, leaf.metric_name));
    }
    if (data.trigger_metric) {
      touch(
        evidenceMatchingLabel(
          data.evidence,
    ...
    if (data.primary_metric) {
      touch(
        evidenceMatchingLabel(
          data.evidence,
          data.primary_metric.metric_code,
          data.primary_metric.metric_name,
        ),
      );
    }
    for (const row of data.metric_comparisons) {
      touch(evidenceMatchingLabel(data.evidence, row.metric_code, row.metric_name));
    }
    return data.evidence.filter((e) => e.document_id != null && !used.has(e.card_evidence_id));
  }, [data]);
  ...
  const relatedCards = filterInsightListCards(data.related_cards);
    ...
    <div className="w-full min-w-0 space-y-5">
      ...
      {data.explanation && (
        <section className="card p-5 md:p-6">
          ...
          <p className="text-[15px] leading-relaxed text-ink">{data.explanation}</p>
          ...
          {showWhyFired && (
            <section className="card p-5 md:p-6">
              ...
              {unmatchedEvidence.length > 0 && (
                <p className="mt-4 text-xs text-ink-soft border-t border-line/50 pt-3">
                  <EvidenceInlineLinks items={unmatchedEvidence} />
                </p>
              )}
              ...
              {data.description && (
                <p
                  className={clsx(
                    ...
                    (unmatchedEvidence.length > 0 || ruleLeaves.length > 0 || data.rule_summary) &&
                      "mt-3",
                  )}
                >
                  {data.description}
          ...
          {relatedCards.length > 0 && (
            <section className="card rounded-xl overflow-hidden">
              <div className="px-4 py-4 border-b border-line/60 bg-surface-2/30">
                <h2 className="text-base font-semibold">Related intelligence</h2>
                <p className="text-sm text-ink-mute mt-0.5">Cards linked to this signal or event.</p>
              </div>
              <div className="p-4 space-y-3">
                {relatedCards.map((c: CardBrief) => (
                  <IntelligenceCard
                    key={c.card_id}
                    card={c}
                    onOpen={setOpenCard}
      ...
      <CardDetailDrawer cardId={openCard} onClose={() => setOpenCard(null)} />
    </div>
  );
}
