...
import { filterQuarterTimelineEvents, groupEventsByQuarter, type TimelineEvent } from "@/lib/events";
...
export function CompanyQuarterTimeline({
  events,
  ticker,
  latestEventId = null,
  collapsible = true,
  ...
  const quarterGroups = useMemo(() => {
    const groups = groupEventsByQuarter(filterQuarterTimelineEvents(events));
            ...
            {expanded && (
              <ol className="relative border-l border-line ml-2 mt-2 space-y-3">
                {group.events.map((ev) => {
                  ...
                  const displayTitle = resolveEventDisplayTitle(ev.event_type, ev.title);
                  return (
                    <li key={ev.id} className="ml-4">
                      ...
                      <button
                        type="button"
                        onClick={() => navigate(`/company/${ticker}/event/${ev.id}`)}
                        className={clsx(
