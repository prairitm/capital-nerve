...
- **Extraction determinism.** Stage 1 ([`services/pipeline/extraction.py`](backend/app/services/pipeline/extraction.COMPONENT.md)) is required to be reproducible: same `(file_hash, PROMPT_VERSION, PARSER_VERSION, provider.name, model, seed)` ⇒ same `ExtractedValue` rows. The mechanism is the request-hash cache on `extraction_jobs.request_hash` (introduced in migration `0005_extraction_cache`). Providers always run at `temperature=0` with a fixed `seed`, use structured output (Anthropic tool use / OpenAI strict `json_schema`), and never silently fall back to the mock. Post-LLM, [`services/pipeline/validators.py`](backend/app/services/pipeline/validators.COMPONENT.md) anchors `source_text` to the page, canonicalises units, and downgrades confidence on totals-math breaches.
