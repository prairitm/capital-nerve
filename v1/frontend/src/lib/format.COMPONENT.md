...
- Exports:
  ...
  - `formatCrAmount(value)` — crore numeric part with 2 decimal places (`CR_FRACTION_DIGITS`).
  - `formatCr(value)` — `formatCrAmount` + ` Cr`; switches to `L Cr` when `|value| >= 100000`.
  - `formatMetricValue(value, unit)` — full string with unit (drawers, evidence).
  - `formatMetricAmount(value, unit)` — numeric part when unit is rendered separately.
