...
/** Full metric value string including unit (drawers, evidence panels). */
export function formatMetricValue(
  ...
  if (u === "Cr" || u.toLowerCase() === "crore") return formatCr(value);
  if (u === "Rs") return `Rs ${formatNumber(value, 2)}`;
  if (u === "days") return `${formatNumber(value, 0)} days`;
  if (u) return `${formatNumber(value, 2)} ${u}`;
  return formatNumber(value, 2);
}

/** Format an intelligence-object metric row (string or numeric value). */
export function formatIOMetricValue(
...
/**
 * Normalize API display strings (e.g. "20616.0 Cr", "12.89%") and numeric
 * values into the standard Indian-market formatting used across the UI.
 */
export function formatDisplayValue(
  ...
  if (crMatch) {
    const num = Number(crMatch[1].replace(/,/g, ""));
    if (!Number.isNaN(num)) return formatCr(num);
  }

  const pctSuffixMatch = trimmed.match(/^([\d,.\-+]+)%\s*(.*)$/i);
  if (pctSuffixMatch) {
    const num = Number(pctSuffixMatch[1].replace(/,/g, ""));
    const suffix = pctSuffixMatch[2]?.trim();
    if (!Number.isNaN(num)) {
      const formatted = formatPct(num, 1);
  ...
  if (unit) {
    ...
    if (!Number.isNaN(num)) return formatMetricValue(num, unit);
  }

  const bareNumber = Number(trimmed.replace(/,/g, ""));
  if (!Number.isNaN(bareNumber) && /^[\d,.\-+]+$/.test(trimmed)) {
    return formatNumber(bareNumber, Number.isInteger(bareNumber) ? 0 : 2);
  }

  return trimmed;
}

/** Numeric display when the unit is rendered separately (snapshot tables, card grids). */
export function formatMetricAmount(
  ...
  if (unit === "Cr") return formatCrAmount(value);
  if (unit === "Rs") return formatNumber(value, 2);
  return formatNumber(value, 2);
}
...
/** Strip internal metric-code parentheticals from feed summaries. */
export function cleanCardSummary(summary: string | null | undefined): string | null {
  if (!summary) return null;
  const cleaned = summary.replace(/\s*\([a-z][a-z0-9_]*=[^)]+\)\s*$/i, "").trim();
  return cleaned || null;
}
