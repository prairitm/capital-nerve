...
/** True when the card reflects a catalog rule that actually fired (not a no-signal placeholder). */
export function isMaterialIntelligenceCard(card: CardBrief): boolean {
  return card.signal_id != null;
}

/** Key intelligence lists: hide quarters where catalog rules did not fire. */
export function filterMaterialIntelligenceCards(cards: CardBrief[]): CardBrief[] {
  return filterInsightListCards(cards).filter(isMaterialIntelligenceCard);
}
